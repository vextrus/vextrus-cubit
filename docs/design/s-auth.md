...
(`s-auth-submit`). Success → the sent notice: **Check your email — we sent you a
verification link.** Refusals here: ACCOUNT_ALREADY_EXISTS, RATE_LIMITED. Footer:
...
block 48 px tall in the body slot (layout kept, no spinner). Done → notice **Your email is
...
Success → notice **Check your email — your sign-in link is on its way.** Refusal:
...
Without `?token=`: field **Email** · submit **Email me a reset link** · success notice
**Check your email — a reset link is on its way.** (same non-disclosure as magic-link).
With `?token=`: field **New password** (`type="password"` `autocomplete="new-password"`,
testid `s-auth-password`) · submit **Set new password** · success notice **Your password is
set and your other devices were signed out.** with footer link **Continue** → `/` —
