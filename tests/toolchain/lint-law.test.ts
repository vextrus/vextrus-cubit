...
describe("AC4: every NEVER fires on its committed fixture", () => {
  test("AC4: the corpus covers exactly the closed rule set", () => {
    const slugs = readdirSync(CORPUS_ROOT).filter((entry) => statSync(join(CORPUS_ROOT, entry)).isDirectory());
    expect([...slugs].sort()).toEqual(Object.keys(RULE_OF_SLUG).sort());
  });
  ...
  test("AC4: every bad fixture reports its own rule", () => {
    const silent = fixtures
      ...
      .filter((fixture) => !(messagesOf.get(fixture.id) ?? []).some((message) => message.ruleId === RULE_OF_SLUG[fixture.slug]))
      .map((fixture) => `${fixture.id} (as ${fixture.virtualPath}) did not report ${RULE_OF_SLUG[fixture.slug]} — it reported ${reported(fixture)}`);
...
describe("AC6: the ARCH-01 matrix is complete branch by branch", () => {
  test("AC6: every direction in the matrix has its own bad fixture", () => {
    ...
    expect([...branches].sort(), "a matrix direction has no fixture proving the rule covers it").toEqual([...MATRIX_BRANCHES, "good"].sort());
  });
