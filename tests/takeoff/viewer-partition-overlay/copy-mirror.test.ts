/**
 * C-13 and B-17 over the one boundary ARCH-01 leaves no shared home for: the views/grid panel lives
 ...
 * tests/takeoff/viewer-inspector/copy-mirror.test.ts does for INSPECTOR_COPY (Decision I-113).
 *
 * A test may import both sides: `tests/**` is outside the layer matrix.
 */
import { describe, expect, test } from "vitest";
import { PARTITION_COPY } from "../../../src/modules/takeoff/viewer-partition-overlay/copy";
import { viewerPartition } from "../../../src/ui/strings/viewer-partition";

describe("the views/grid panel's mirrored copy is the registry's own", () => {
  test("every mirrored sentence is the registry's value, byte for byte", () => {
    const registry = viewerPartition as unknown as Record<string, string>;
    for (const [key, mirrored] of Object.entries(PARTITION_COPY)) {
