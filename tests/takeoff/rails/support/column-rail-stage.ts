...
/** The fixture this rail is graded against unless a caller names another (F-RCC6, AM-01's regression). */
export const DEFAULT_GOLDEN_FIXTURE = "rcc6";

/** Where a fixture keeps its golden takeoff: `fixtures/<id>/takeoff.golden.json` (AM-01: two fixtures). */
 ...
 * formula it came from. Schema 2 (F-RCC6-BNBC, AM-01) keys the same way — `kind` there reaches
 * beyond RCC_CONCRETE/FORMWORK to REBAR, PILE_LENGTH, PILE_COUNT, EXCAVATION, BLINDING and
 * BRICKWORK — and carries the grade, the bar diameter, the component the quantity is (NET, LAP,
 * EDGE) and the members it came from. The optional fields are exactly the ones schema 1 omits, so
 * one type admits both and a reader that only knows schema 1 still reads a schema-2 row.
 */
...
/** The golden takeoff of `fixtureId` (default F-RCC6: every caller that named none reads what it always did). */
