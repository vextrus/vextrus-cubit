...
import { GOLDEN_FIXTURE as GOLDEN_FIXTURE_PATH, goldenRows as goldenRowsOf } from "../../../golden/support/golden-fixture";
import {
  COLUMN_C1,
  MEASURED,
...
// The golden reader lives in the golden lane's own support (ARCH-02: one home), because reading a
// committed golden must not pull this stage's live-database imports in behind it. Re-exported here
// so every caller of `goldenRows()` reads the same door it always did (PREMISES C6).
export {
  ...
  type GoldenDocument,
  type GoldenRow,
} from "../../../golden/support/golden-fixture";
...
export function goldenColumnConcreteByLevel(): Map<string, string> {
  ...
  const rows = goldenRowsOf().filter((row) => row.class === wanted.class && row.kind === wanted.kind);
  expect(rows.length, `${GOLDEN_FIXTURE_PATH} records column concrete rows (it records ${goldenRowsOf().length} rows in all)`).toBeGreaterThan(0);
