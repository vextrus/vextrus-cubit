...
function holePerimeterMm(hole: { area: string; rect?: readonly string[]; poly?: readonly (readonly string[])[] }, units: Canon): string {
  ...
  // A hole the model outlines not at all: the perimeter a SQUARE of that area would be cut by. Among
  // rectangles of one area the square has the SMALLEST perimeter — 2(L + W) ≥ 4·√(L·W) — so `4·√area`
  // is a LOWER bound on that hole's reveal and the allowance built on it is deliberately understated.
  // That is the only direction open: an understated allowance RAISES the band floor and makes the case
  // stricter, while an upper bound would buy slack for over-measurement, which L-QTY-06 ("±3% under,
  // +0% over") and L-QTY-04 ("Over-measurement → hard block, never a disclosure") both refuse. The
  // other cure — inventing an outline from the area — is the bounding-boxing L-MEA-03 forbids outright.
...
/**
 * The holes F-RCC6-BNBC's slabs are cut by, as the fixture states them: how many in all, and how many
 * state no outline at all and so fall to `holePerimeterMm`'s understated `4·√area` floor.
 *
 * The fixture is BYTE-FROZEN (AM-01: "Two fixtures, never a replacement"), so these two counts are
 * properties of the yardstick itself and not a roster a later increment extends. They are pinned
 * because the understated branch is safe only while it stays this small: a regenerated model that
 * handed it a long thin hole — 0.1 m × 10 m, perimeter 20.2 m against the square's 4 m — would shrink
 * the reveal allowance far below the reveal the golden priced and drive S under 0.97 × G for a reason
 * no assertion names. Pinned here, such a fixture fails by name in the transcription instead.
 */
export const BNBC_SLAB_HOLES = 74;
export const BNBC_UNOUTLINED_SLAB_HOLES = 5;

/** The two counts above, taken from the model itself each time the fixture is transcribed. */
export function bnbcHoleOutlineRoster(): { holes: number; unoutlined: number } {
  const holes = bnbcModel()
    .members.filter((member) => member.class === "SLAB")
    .flatMap((member) => [...(member.holes ?? [])]);
  const unoutlined = holes.filter((hole) => hole.rect === undefined && hole.poly === undefined);
  expect(holes.length, `${BNBC_MODEL} states the ${BNBC_SLAB_HOLES} slab holes this acceptance was written against — a fixture is never replaced (AM-01)`).toBe(BNBC_SLAB_HOLES);
  expect(
    unoutlined.length,
    `${BNBC_UNOUTLINED_SLAB_HOLES} of them state no outline, so their reveal is taken at the square's understated floor (holePerimeterMm): a model that hands that branch a hole of another shape understates the allowance by more than the band can absorb, and says so here rather than as an unexplained shortfall against the golden (AM-01, L-QTY-06)`,
  ).toBe(BNBC_UNOUTLINED_SLAB_HOLES);
  return { holes: holes.length, unoutlined: unoutlined.length };
}
