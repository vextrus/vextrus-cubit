...
describe("AC-4: the conventions stage runs after the views stage", () => {
  test("AC-4: the stage list is the views stage and then the convention profile", async () => {
    ...
    // Presence and order, never closure: R-TO-030 schedules the grid backbone (L-CAD-07) into this
    // same roster, so an assertion about how long the list is would be an assertion about a date
    // rather than about the product (B-19, C-05).
    ...
    expect(
      stages.filter((stage) => !said.includes(stage)),
      `every stage the list names is one that really runs — a stage with nothing behind it records no step of its own; the log reads: ${said.join(" → ")}`,
    ).toEqual([]);
    expect(stages[0], "R-TO-030's stored partition runs the view classification first (L-CAD-06)").toBe(VIEWS_STAGE);
    ...
    expect(
      ...
      `and it runs after the view classification — the census it resolves from is taken over the views that stage derived; the list reads: ${stages.join(" → ")}`,
