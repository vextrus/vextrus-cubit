...
describe("AC-4: the SITE-fact ledger is migrated, tenant-scoped and append-only", () => {
  ...
  it("AC-4: the ledger is append-only against its OWNER too, by row and by TRUNCATE", async () => {
    ...
    expect(
      triggers.some((definition) => /BEFORE UPDATE OR DELETE/i.test(definition) && /cubit_append_only/i.test(definition)),
      `a BEFORE UPDATE OR DELETE trigger runs cubit_append_only() — the belt every ledger of this tree wears (L-ACT-03); it carries ${JSON.stringify(triggers)}`,
    ).toBe(true);
    expect(
