...
export const DEFERRED_CODES: Readonly<Record<string, string>> = Object.freeze({
  ...
  // behaviour itself.
  // The draft's own three. Two of them are raised by doors that read the store, so their proof is a
  // live-database one and the file that holds it is named: tests/takeoff/boq/export-door-guard.db
  // .test.ts asks both by name over a scratch database the migrations built. That lane is not the one
  // the register reads (vitest.config.ts), so what is owed is an assertion in the EXECUTED lane and
  // never the behaviour — exactly as the eleven below. The third owes the behaviour itself, and says so.
  BOQ_NO_CAMPAIGN:
