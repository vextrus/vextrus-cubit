...
export const DEFERRED_CODES: Readonly<Record<string, string>> = Object.freeze({
  ...
  REBAR_SCHEDULE_UNREAD:
    "the increment that lands the remaining rebar readers — the REBAR area raises the code today (src/modules/takeoff/rebar/bars.ts) for every class that BEARS `rcc.rebar` and has no reader yet, and in src/core/rulesets/methods/rebar/stock.ts for a lap at or beyond the stock bar. Neither arm is reachable from the executed lane: this leaf reads the two vertical classes, and F-RCC6-BNBC stages columns only, so every member the corpus offers is a member whose schedule WAS read. What is owed is a member of a borne class with no reader to be observed, which arrives with the beam, slab and foundation readers; that increment owns both halves.",
  LINTEL_SOURCE_ABSENT:
    "the opening-schedule reader increment — the FRAME area's lintel rail raises the code today (src/modules/takeoff/rails/frame/lintel.ts) and the area's own acceptance asserts it (tests/takeoff/rails/frame-rails-offer.test.ts), but it asserts it through a for-of binding over the roster, which is one alias hop further than the scan follows, so the executed lane holds the claim and cannot show it. What is owed is a lintel with a SOURCE to be absent from: no F-RCC6 opening states one, so the rail reports the code for every lintel placement and nothing yet exercises the arm where a stated source is read instead. The increment that lands the opening-schedule reader owns both halves.",
});
