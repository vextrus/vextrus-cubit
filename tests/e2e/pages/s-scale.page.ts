...
export class SScalePage {
   ...
   * journey opens the control and presses the option the way a reader does, by the option's own
   * accessible name.
   */
  async select(testId: string, option: string): Promise<void> {
    ...
    // One locator, so the choice is a retried read: asking how many options matched and branching on
    // the answer is a one-shot reading of a screen that is still opening (the lane's own lint law).
