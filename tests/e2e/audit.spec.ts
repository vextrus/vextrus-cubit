...
/**
 * The relation the model-ledger panel probes for. Its one home is `AUDIT_PANEL_TABLES.modelLedger`
 * in src/modules/spine/audit, which a journey may not import — that module opens the database seam,
 * and a Playwright worker loading it would connect a driver from the test process.
 */
const MODEL_LEDGER_TABLE = "model_calls";

/**
 * Does this installation hold the model ledger? The panel's posture is a function of that and
 * nothing else, so the journey asks the same question of the same lane database rather than
 * freezing whichever answer today's cluster happens to give (B-19 — arming by input existence).
 *
 ...
 */
function modelLedgerExists(): boolean {
  return isTrue(scalar(e2eDatabaseUrl(), `select to_regclass('${MODEL_LEDGER_TABLE}') is not null;`));
}
...
test.describe("J-003 — the project's audit surfaces", () => {
  test("J-003: the act log explorer stands on a project, with the ledger and jobs panels disarmed", async ({ page, baseURL }, testInfo) => {
    ...
    const ledgerArmed = modelLedgerExists();
    await expect(
      audit.modelLedger,
