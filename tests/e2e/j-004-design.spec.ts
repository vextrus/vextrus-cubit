 ...
 *
 * What the scan gates on is the lane's own helper, unchanged: Q-11 states one threshold —
 * "axe: zero serious/critical on every journey checkpoint" — R-UI-012 repeats it ("axe
 * serious/critical = 0 on every screen"), and `expectNoAxeViolations` in tests/e2e/axe.ts
 * asserts something strictly stronger: an empty violation list on axe's full rule set, of any
 * impact at all. Calling it here subsumes both clauses instead of restating them, and it is
 * the same helper J-000's checkpoints call, so one lane means one bar. Nothing is narrowed:
 * no rule is disabled, no violation is filtered away, no mask is added and no checkpoint is
 * skipped or moved — the overlay-open state is a legitimate state under R-UI-011 and is
 * scanned with the surface up.
 *
...
/**
 * Scan one checkpoint with the lane's own helper, having first proved there is a sheet to
 * scan.
 *
 * The guard is the whole reason this is a function rather than a bare call: axe finds nothing
 * on a page that never rendered, so a checkpoint that quietly lost its sheet would pass the
 * scan hollowly and this journey would keep reporting thirty-six greens for nothing. The root
 * is asserted first — it is present in every state, including `loading` — and then
 * `expectNoAxeViolations` does the scanning, unnarrowed, on the full rule set, exactly as
 * J-000's own checkpoints call it. The checkpoint's name rides along so a red says which
 * framing it was before the helper's message says what was wrong with it.
 */
async function scanCheckpoint(page: Page, checkpoint: string): Promise<void> {
  await expect(
    page.getByTestId('design-gallery-root'),
    `${checkpoint}: the sheet never rendered, so its scan would have proved nothing`,
  ).toBeVisible();
...
test.describe('J-004', () => {
  for (const theme of THEMES) {
    test(`live-gallery (${theme}): the sheet is axe-clean and its module sections hold their baselines`, async ({
      ...
      // state its entry declares, scanned at once by the lane's own helper.
      await scanCheckpoint(page, `live-gallery (${theme})`);
    });
  }
  ...
  for (const state of SCREEN_STATES) {
    for (const theme of THEMES) {
      test(`state-${state} (${theme}): the forced R-UI-050 state holds its baseline and is axe-clean`, async ({
        ...
        await scanCheckpoint(page, `state-${state} (${theme})`);
      });
    }
  }
  ...
  for (const entry of OVERLAY_ENTRIES) {
    for (const theme of THEMES) {
      test(`overlay-${entry} (${theme}): opened by keyboard, photographed and axe-clean`, async ({
        ...
        await scanCheckpoint(page, `overlay-${entry} (${theme})`);
      });
    }
  }
