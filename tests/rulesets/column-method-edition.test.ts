...
describe("AC-6: the column method is enumerated, implemented and cited", () => {
  ...
  test("AC-6: the platform seed is IS1200_IN @ 2026.12 and cites every method the shards enumerate", async () => {
    const registry = await methodsRegistry();
    ...
    // Re-baselined by the masonry leaf: its three methods are minted at the next version beside the
    // standing 2026.11 rather than over it — an edition is immutable (L-MEA-01, B-20).
    expect(SEED_VERSION, "and the version it is re-minted at is the one this increment lands").toBe("2026.12");
