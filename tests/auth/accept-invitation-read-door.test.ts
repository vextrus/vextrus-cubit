...
/**
 * The five doors that were limited before this one, with the allowances they were limited by. AC-1
 * says they are unchanged and the table gains exactly one key, so both halves are stated here; a
 * later increment that lawfully moves one of these figures re-baselines this case with it (B-20).
 */
...
test("AC-1: the doors that were already limited are untouched, and the table gained exactly one key", () => {
  ...
  expect([...doors.keys()].sort(), "one door added, none dropped and none renamed").toEqual(
    [...Object.keys(DOORS_ALREADY_LIMITED), "acceptInvitationRead"].sort(),
  );
});
