...
// In `--scratch` form this script *is* the verify chain's schema-drift lane, and verify has already
// printed that lane's roster line; standalone it owns its own line (C-06's stdout contract).
if (scratchMode && stage.status === "stub") {
  announce({ ...stage, id: "schema-drift" });
  process.exit(0);
}
