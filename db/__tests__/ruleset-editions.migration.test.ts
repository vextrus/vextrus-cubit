 ...
 *
 * Re-baselined under B-20 by the slab, shear-wall and stair leaf, which mints a THIRD row. The
 * reading is the one this case always made — a re-mint is "inserted BESIDE the immutable row, never
 * over it" — and it is the roster that grows, not the discipline: the store holds one row per mint,
 * and an edition that had been edited in place would leave a version missing here.
 */
