 ...
 * masonry and rebar leaves, each of which mints one more. The reading is the one this case always
 * made — a re-mint is "inserted BESIDE the immutable row, never over it" — and it is the roster that
 * grows, not the discipline: the store holds one row per mint, and an edition that had been edited
 * in place would leave a version missing here.
 */
const MINTED_SEED_VERSIONS: readonly string[] = [SEEDED_SEED_VERSION, "2026.09", "2026.10", "2026.11", SUPERSEDED_SEED_VERSION, SEED_VERSION];
