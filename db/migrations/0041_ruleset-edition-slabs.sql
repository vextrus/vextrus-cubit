-- L-MEA-01, B-20: the platform rule set re-minted as `IS1200_IN @ 2026.10`, beside the rows 0004
-- seeded and 0036 minted, and never over either.
--
-- An edition is immutable: a method landing in the tree is a NEW edition rather than an edit to the
-- standing one. 2026.08 and 2026.09 stand untouched as the rows every campaign opened under them
...
--
-- What it cites is every method the shards enumerate (`enumerateMethods()`): the three 0036 cited
-- and the twelve this leaf lands for slabs, shear walls and stairs. A pair the tree can compute and
-- no campaign could cite would be a method in force that nothing in force names. The seventeen
-- parameter values are 0004's, unchanged — this edition differs from its parent in its methods
-- alone. `content_digest` is `editionDigest` over exactly the content src/core/rulesets/seed
...
-- the snapshot beside this entry says exactly what 0040's does.
--
...
INSERT INTO "ruleset_editions" ("scope", "name", "version", "content_digest", "parameters", "methods")
	VALUES (
		...
		'IS1200_IN',
		'2026.10',
		'da035655a5d04176f34913ed1bacc6505042defd09543f93fad4374187601fdd',
		'{
