-- L-MEA-01, B-20: the platform rule set re-minted as `IS1200_IN @ 2026.12`, beside the row 0044
-- minted and never over it.
--
-- An edition is immutable: a method landing in the tree is a NEW edition rather than an edit to the
-- standing one. 2026.08, 2026.09, 2026.10 and 2026.11 stand untouched as the rows every campaign
...
-- refuse it, and that refusal is the point.
--
-- What it cites is every method the shards enumerate (`enumerateMethods()`), which is now nineteen
-- pairs: the sixteen 2026.11 cited and the three the MASONRY area records — L-MEA-02's brickwork by
-- nominal thickness, and L-MEA-03's plaster and paint of a surface. A pair the tree can compute and
-- no edition cites is a method in force that nothing in force names. The seventeen parameter values
-- are 0004's, unchanged — this edition differs from its parent in its methods alone, and the two
-- deduction thresholds this area partitions against (`openingDeductionMinM2` for a wall's openings,
-- `finishOpeningDeductionMinM2` for a finished face's) are read from exactly these values as DERIVED.
-- `content_digest` is `editionDigest` over exactly the content src/core/rulesets/seed exports, so a
-- stored digest that disagreed would mean the row and the module hold different content.
--
...
SELECT set_config('cubit.system_reason', 'migration 0046 mints the platform rule-set edition IS1200_IN @ 2026.12', false);--> statement-breakpoint
INSERT INTO "ruleset_editions" ("scope", "name", "version", "content_digest", "parameters", "methods")
	VALUES (
		'platform',
		'IS1200_IN',
		'2026.12',
		'04364cb8858499f79d489cb450b472136821fd51c502518929c29453b04328df',
		'{
