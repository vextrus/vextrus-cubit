...
-- them measured against, and this row is the head every pin minted afterwards forks (L-REG-07). Nothing
-- here touches the earlier rows — the append-only trigger 0004 installed would refuse it, and that
-- refusal is the point.
--
-- What it cites is every method the shards enumerate (`enumerateMethods()`), which is now twenty-one
-- pairs: the nine 2026.10 cited and the twelve the SLABS area records for R-TO-032's plates, shear
-- walls and stairs. A pair the tree can compute and no edition cites is a method in force that nothing in
-- force names. The seventeen parameter values are 0004's, unchanged — this edition differs from its
-- parent in its methods alone. `content_digest` is `editionDigest` over exactly the content
...
-- the snapshot beside this entry says exactly what the one before it does.
--
...
SELECT set_config('cubit.system_reason', 'migration 0043 mints the platform rule-set edition IS1200_IN @ 2026.11', false);--> statement-breakpoint
INSERT INTO "ruleset_editions" ("scope", "name", "version", "content_digest", "parameters", "methods")
	VALUES (
		'platform',
		'IS1200_IN',
		'2026.11',
		'd31442bda6ed57ab23801cc83cbb9ab72cad70958a881a811ebac0ea2b9a7af9',
		'{
