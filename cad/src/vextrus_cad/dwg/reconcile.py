...
A shortfall does not refuse the sheet and never raises. It names the class it lost and the space
it lost it on, and the caller carries the list.
"""
...
def reconcile(
    ...
    Pure: nothing is read from disk and neither tally is touched. A class the conversion matched
    or over-produced is no loss, and a space the census never named can be short of nothing.
    """
    ...
    for space, types in census.items():
        ...
        for dxftype, counted in types.items():
            ...
            if dxftype == UNKNOWN_ENT:
                # An unnamed class is refused for what it is, not for how many of it survived:
                # the shortfall arithmetic below cannot speak about a class nothing can name.
