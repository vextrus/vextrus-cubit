...
def geometry_tally(dxf_path: Path) -> dict[str, dict[str, int]]:
    ...
    with held_library_words() as words:
        try:
            ...
            raise DwgError(
                f"the converted DXF {path.name} cannot be read: {error}{said(words)}"
            ) from error
        except (ezdxf.DXFError, ValueError) as error:
            # A conversion this reader cannot walk is an unparseable DXF whichever half of it
            # failed: ezdxf refuses the file, or the drawing inside it holds a value nothing can
            # be read from.
            raise DwgError(
                f"unparseable converted DXF {path.name}: {error}{said(words)}"
            ) from error
