...
travels to `logging.lastResort`, which writes it straight onto this process's stderr, and a
library warning would do the same. Both are held here for the duration of a read instead. What was
said is not thrown away — it is handed back to the caller, which folds it into the refusal that
names the drawing (L-CAD-04's loud failures), rather than leaking beside the answer.
"""
