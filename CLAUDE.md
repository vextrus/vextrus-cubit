...
- Forward-only pin AC: the shared file is locked, the fix lives downstream — An increment repairing a pinned regression can carry an AC that checks every member the pin tracked in a shared page object or e2e support file is still there, in order — and that same file is often Verifier-authored and locked, so writing to it is refused.
