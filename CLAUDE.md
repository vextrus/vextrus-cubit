...
- The held-out mount cannot resolve the product's @/ alias — mock at the specifier string, which vitest answers from its registry — Verified 2026-09-07 (that increment, `heldout_dryrun`), after that increment re-spelled `src/**` imports through tsconfig's `@/` alias: - `productModule("src/app/api/events/route.ts")` dies with `Error: Cannot find package '@/core/jobs' imported from …
