...
- Restore from a ref names files, not directories — A session tried to restore state from a ref using a directory path or a glob, and was denied four times in one increment. The hook's own words: a restore from a ref names explicit files — `.`, globs and directories can reach the acceptance.
...
### Tests & acceptance
- Trace/viewer-shaped increments: don't retake baselines while journeys are still moving — An increment that touches both a traced UI surface (a viewer/inspector's rendering) and its journey specs can be tempted to re-take the design baseline to make a snapshot check pass, even though the underlying journey assertions (what the golden path or a …
