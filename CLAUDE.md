...
- The cad lane (Python, uv): its reads are allowlisted as the verify chain runs them — `uv run --project cad pytest cad`, `ruff check cad`; `dwgread <f.dwg>` prints to stdout; `dwg2dxf -m -o <scratch>/x.dxf <f.dwg>` writes to scratch only (plain `dwg2dxf` writes beside its input — denied; `-m` is the form ezdxf reads).
- A Verifier-authored file — a BINARY fixture like a `.dwg` included — never yields to a Builder or Fixer. Say so ONCE as an Objection in your handoff and build around it; after two denials the engine raises the dispute itself, with the toolchain's reading of a binary fixture.
- The Verifier never `git commit`s — the engine makes the `verifier:` commit; leave the tree dirty.
- Session memory: one file at `~/.claude/projects/-home-riz-vextrus-cubit/memory/<name>.md` (frontmatter name/description/type), by Write or Bash — the one admitted path under `~/.claude/`; the engine harvests it after each merge. The rest of `~/.claude/` is locked.
