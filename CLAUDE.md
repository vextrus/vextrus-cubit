...
- Verifier acceptance that spawns a lane (pnpm e2e / verify) belongs in db/__tests__; the gate asks it through db/__tests__/vitest.config.ts.: When an increment's own deliverable is a test lane (e.g. inc-007c ships `tests/e2e/j-004-design.spec.ts`), the Verifier cannot put acceptance in the increment's ownership paths — writing there locks the Builder out — and
<!-- builder:lessons:end -->
