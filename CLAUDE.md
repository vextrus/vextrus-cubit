...
- A read-only role cannot shell a generator or a toolchain probe — Trigger: a planner or reviewer turn tries to produce or measure an artifact by running it — a fixture generator under `uv run --project ... python`, `dxf2dwg -o ...`, `python3 - <<'EOF'`, or a bare `python -V` timing.
### Tests & acceptance
...
- A test that cannot run yet is an Objection, not a skip — Trigger: an increment that lands binary or generated fixtures writes acceptance tests ahead of the bytes and marks them `.skip` (or narrows with `.only` while iterating) so the suite goes green.
### Screens & design
...
- Three LibreDWG 0.13.3 round-trip traps met authoring F-RCC6: an R2000 DXF leaks every dimension block after the first into model space (author R2004), ezdxf's … — Measured 2026-09-03 (LibreDWG 0.13.3, ezdxf 1.4.4) while building `fixtures/gen/rcc6.py` (F-RCC6: 8 paper layouts, 1105 originals, 29 DIMENSIONs, INSERT title blocks, MTEXT notes): - **Dimension-block leak.** From an ezdxf **R2000** DXF, `dxf2dwg` → …
<!-- builder:lessons:end -->
