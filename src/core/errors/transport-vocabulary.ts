...
export const TRANSPORT_VOCABULARY: ReadonlyArray<TransportVocabulary> = Object.freeze([
  ...
  Object.freeze({
    ...
    // EnumLabel — never with this name. Its home is `../notes/law.ts` (ARCH-02), and it is READ from
    // there rather than copied, so a kind the law gains tomorrow is declared with no edit (B-19).
    vocabulary: "note kinds (R-TO-034)",
    codes: NOTE_KINDS,
  }),
  Object.freeze({
    // L-MEA-05's scale ranks, spelled as the law spells its precedence: "QS two-point › grid-spacing
    // match › overridden dimension ratio › file units header". A rank names what an affirmation stood
    // on; it is written to the affirmation and read by the engine, and a person is answered with
    // `SCALE_NO_EVIDENCE` or `SCALE_UNIT_UNMAPPED`, never with one of these. Its home is
    // `../scale/law.ts` (ARCH-02).
    vocabulary: "scale ranks (L-MEA-05)",
    codes: Object.freeze(["QS_TWO_POINT", "GRID_SPACING", "DIMENSION_RATIO", "FILE_UNITS"]),
  }),
  Object.freeze({
  ...
  Object.freeze({
    ...
    // EntityGraph mirror that parses them, and by the model seam's key grammar in
    // `src/core/model/sources.ts`, which resolves a proposal's citations (L-AI-02). A scheme names
    // who read the file bytes; it rides on an artifact key and is never answered to a user.
    vocabulary: "EntityGraph source-key schemes (L-CAD-02)",
    codes: Object.freeze(["DXF_HANDLE", "PDF_OBJECT", "RASTER_TRACE"]),
  }),
  Object.freeze({
