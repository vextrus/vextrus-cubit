 ...
 *
 * Re-baselined for THREE ADDED entries and nothing else: `EXPORT_URL_INVALID`, `EXPORT_URL_EXPIRED`
 * and `EXPORT_NOT_FOUND` (./exports.ts), the three the download door of R-SPINE-041's export seam
 * answers with when the link itself is the problem — a link this installation did not sign, a link
 * whose hour has passed, and an address this workspace has nothing stored at. The roster grew by
 * those three keys — 77 codes to 80 — and not one existing entry's code, message, remedy, severity or
 * surface moved with them; the previous digest was
