...
/**
 * The figure a person's own words state, canonicalised the one way the grammar canonicalises one — so
 * a reading somebody EDITED is comparable with the reading the grammar proposed, rather than being
 * compared as two strings that happen to differ in a comma (R-TO-034, L-REG-04).
 *
 * The digits and the decimal, and nothing else: what stands around them is the words the reading is
 * kept verbatim in, beside this (L-QTY-01).
 */
export function canonicalFigureOf(written: string): string {
  return new RegExp(FIGURE).exec(written.trim())?.[0].replace(/,/g, "") ?? written.trim();
}
 ...
 *
 * A sentence is read as the drawing wrote it and the proposal keeps it verbatim, so what a person is
 * shown is the drawing's own line (L-CAD-03).
 */
