...
export const PERMISSIONS = [
  ...
  "PRICE",
  "BID",
  // AM-04 cuts a fourteenth: minting a rule-set edition is its own permission, so L-MEA-01's
  // "authoring mints a new edition, never updates one, and is its own permission" is true of the
  // enum rather than of a comment. Authoring a rate library's parameter values is untouched and
...
export const ACT_PERMISSION: Readonly<Record<ActType, Permission>> = Object.freeze({
  ...
  // AM-04: "L-ACT-03's closed permission enum gains AUTHOR_RULE_SET, carrying the act type
  // AUTHOR_RULESET_EDITION". This replaces the staged reading that used ADMINISTER_PROJECT.
...
export const ROLE_PERMISSIONS: Readonly<Record<Role, readonly Permission[]>> = Object.freeze({
  ...
  // PRINCIPAL takes it by holding the whole enum, so LEAD is the only bundle that is written.
