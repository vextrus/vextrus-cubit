...
//
// Every value below is `src/ui/strings/schedules.ts` verbatim, and
// `tests/takeoff/notes/copy-mirror.test.ts` is what makes that a fact. The cure — a copy home both
// layers may read — is recorded with its owner in docs/design/s-schedules.md §8.
...
export const SCHEDULES_COPY = Object.freeze({
  ...
  schedules_offline:
    "You are offline. The sheets read as they stood when this page loaded, and nothing can be committed until the connection returns.",
