...
export async function railSetupOf(scope: RailSetupScope): Promise<RailSetup> {
  ...
  return {
    ...
    grades: {},
    // The runs the partition read for this campaign's beam and tie-beam placements, and the openings
    // an opening schedule states behind its lintels. Both are read from the partition's own store;
    // until the loader below fills them a frame rail reports RUN_UNREAD or LINTEL_SOURCE_ABSENT
    // rather than measuring a member on a figure nobody read (L-MEA-09, L-QTY-01).
    runs: {},
    lintels: {},
  };
}
