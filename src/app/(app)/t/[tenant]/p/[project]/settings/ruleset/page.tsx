...
export default async function ProjectRulesetSettings({ params }: { params: Promise<{ tenant: string; project: string }> }) {
  ...
  // The two-pane template (Design Direction 00 §3.6) is the LAYOUT's now (settings/layout.tsx): the
  // project's settings areas on the left, this screen's own content on the right. What is left here
  // is the screen, which is the same component a suite mounts on its own.
