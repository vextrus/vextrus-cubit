// S-Levels (R-TO-033): the level stack, read straight through the module's own door — the live
// levels, how each one's height stands, and what the campaign's stored lines amount to on each.
//
// Thin by design: the reading is `levelsViewOf`'s, what a reader may do here is L-ACT-03's own
...
export const metadata = { title: strings.takeoff_nav_levels };

/** The file route this screen answers at, as the fault seam and the state matrix both key it. */
const ROUTE = "/t/[tenant]/p/[project]/takeoff/levels";

/**
 * The three permissions the doors on this screen move (Decision §2's denial table). The reading is
 * L-ACT-03's own, so the screen discloses exactly what the act seam would enforce and never a second
 * opinion of it (B-17, I-50).
 */
const DOOR_PERMISSIONS = ["AUTHOR_LEVEL_STACK", "AUTHOR_PROJECT_FACT", "MEASURE"] as const;
...
export default async function ProjectLevels({ params }: { params: Promise<{ tenant: string; project: string }> }) {
  ...
  // absence, not an empty stack and not a permission short of one (R-UI-050 asks each state to say
  // the true thing).
  const { tenantId, userId } = await authorizePage({ tenant, project });

  const permitted = await permissionsFor(tenantId, project, userId);

  // A read that fails is a fault, not an empty stack: it is recorded once, at the one seam that
  // mints a report id, and the screen quotes that id beside its retry (ARCH-03, B-21).
  let view: LevelsView | null = null;
  let reportId: string | null = null;
  try {
    view = await levelsViewOf({ tenantId, projectId: project });
  } catch (cause) {
    reportId = reportFault({ requestId: crypto.randomUUID(), actor: userId, route: ROUTE, cause }).faultId;
  }

  return <LevelsScreen view={view} projectId={project} permitted={permitted} reportId={reportId} />;
}
