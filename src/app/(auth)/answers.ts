 ...
 *
 * `search` is the query the screen was reached with, where the screen has one. A missing detail is
 * resolved on the very screen that asked for it, and a screen reached through a mailed link is only
...
/**
 * A screen's query as an address carries it. A screen with nothing in its query is linked to as
 * itself, with no empty `?` left hanging on the end of it.
 */
