...
import { z } from "zod";
import { EXPORT_KINDS, MIME_OF_KIND, signedStorageUrl, type ExportKind } from "@/core/exports";
...
export const GET = routeHandler({ route: ROUTE, actor: "exports", schema: ASKED, sentence: NOT_A_LINK }, async ({ input, context }) => {
  ...
  // The signature, read back by the seam that minted it, at the address it was minted for: a link
  // signed for another workspace or another artefact verifies as the forgery it is.
  const storage = appStorage();
  const verified = storage.verify(signedStorageUrl({ tenantId: tenant, sha256: id, expires, signature }));
  if (!verified.ok) return refusalAnswer(verified.reason === "expired" ? "EXPORT_URL_EXPIRED" : "EXPORT_URL_INVALID");

  const bytes = await storage.get(tenant, id);
  if (bytes === null) return refusalAnswer("EXPORT_NOT_FOUND");
  return artefactAnswer(bytes, id, kind);
});
