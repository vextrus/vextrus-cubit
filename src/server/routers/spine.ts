...
async function actorFor(userId: string, projectId: string, actType: typeof ASSIGN_PARTICIPANT_ROLE | null): Promise<ActorCtx> {
...
export const participantsRouter = router({
  roleHistory: signedInProcedure
    ...
    .query(async ({ ctx, input }) => {
      const actor = await actorFor(ctx.session?.userId ?? "", input.projectId, null);
  ...
  assignRolePreview: signedInProcedure
    ...
    .query(async ({ ctx, input }): Promise<{ consequence: Consequence; consequenceDigest: string }> => {
      const actor = await actorFor(ctx.session?.userId ?? "", input.input.projectId, ASSIGN_PARTICIPANT_ROLE);
