...
// the routed screen, and each of them needs to know whose session it is; resolving the cookie once
// per asker cost three round trips for one page view, and `resolveSession` writes
// `sessions.lastSeenAt` when the resolution window has passed — so a plain read could issue the
