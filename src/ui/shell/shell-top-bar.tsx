...
export function ShellTopBar({ workspace, area, atAreaHome, page, email, signOut }: ShellTopBarProps) {
  ...
  return (
    <header className="cx-shell-topbar" data-testid="shell-topbar">
      ...
      {/* The bar's right-hand cluster, in R-UI-030's own order: the ⌘K trigger, the jobs tray
          R-UI-030 names among the frame's parts, then the user menu. Both occupants are
          provider-gated and render nothing outside their provider, so a bar mounted without the
          tenant frame above it stands exactly as it always has (I-116, I-135). */}
